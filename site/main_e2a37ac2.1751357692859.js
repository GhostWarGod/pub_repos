...
function WalletComponent() {
  ...
  const handleTransaction = async () => {
    ...
    // // 查询子钱包地址
    // const walletAddressRes = await client.runMethod(masterContract, 'get_wallet_address', [{ type: 'slice', cell: beginCell().storeAddress(myAddress).endCell() }]);
    ...
    const payload = beginCell().storeUint(0xf8a7ea5, 32) // Jetton transfer opcode
    ...
    .storeAddress(Address.parse(recipientAddress)).storeAddress(Address.parse(wallet.account.address)).storeBit(0).storeMaybeRef(null).storeCoins(toNano("0.02")) // forward gas
